	...
	schema: {
		body: {
			content: {
				...
				'application/xml': {
					schema: Type.Union([Type.Literal('array of const 1'), Type.Literal('array of const 2')]),
				},
			},
		},
		response: {
			...
			'4xx': {
				content: {
					'application/json': {
						schema: Type.Object({
							errorCd: Type.Optional(Type.Number()),
							errorMessageTx: Type.Optional(Type.String()),
						}),
					},
				},
			},
